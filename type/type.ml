...
and type_var = {
  type_var_id      : Id.id;
  type_var_kind    : Kind.Inner.kind;
  type_var_flavour : Kind.Inner.flavour;
}
...
(* TODO: Figure out why this was redeclared from Kind.Inner.flavour? *)
(* and flavour = Kind.Inner.flavour
 *   | Meta 
 *   | Skolem
 *   | Bound *)
...
and Show_type_var : BasicClasses.Show with type t = type_var = struct
  ...
  let show s = Printf.sprintf "{ type_var_id : %s; type_var_kind : %s; type_var_flavour : %s }"
                 (Id.Show_id.show s.type_var_id) (Kind.Inner.Show_kind.show s.type_var_kind) (Show_flavour.show s.type_var_flavour)
end

and Show_flavour : BasicClasses.Show with type t = Kind.Inner.flavour = struct
  open Kind.Inner
  type t = flavour
  let show = function
    | Meta -> "Meta"
...
and Show_type_con : BasicClasses.Show with type t = type_con = struct
  ...
  let show s = Printf.sprintf "{ type_con_name : %s; type_con_kind : %s }"
                 (Name.show_name s.type_con_name) (Kind.Inner.Show_kind.show s.type_con_kind)
end
...
and Show_type_syn : BasicClasses.Show with type t = type_syn = struct
  ...
  let show s = Printf.sprintf "{ type_syn_name : %s; type_syn_kind : %s; type_syn_rank : %s; type_syn_info : %s }"
                 (Name.show_name s.type_syn_name) (Kind.Inner.Show_kind.show s.type_syn_kind)
...
and Show_syn_info : BasicClasses.Show with type t = syn_info = struct
  ...
  let show s = Printf.sprintf "{ name : %s; kind : %s; params : %s; typ : %s; rank : %s; doc : %s }"
                 (Name.show_name s.name) (Kind.Inner.Show_kind.show s.kind)
                 (* (List.to_string s.params ~f:(fun e -> show e)) *) "[params]"
                 (Show_typ.show s.typ) (string_of_int s.rank)
                 s.doc
end

module Eq_flavour : BasicClasses.Eq with type t = Kind.Inner.flavour = struct
  open Kind.Inner
  type t = flavour
...
let tcon_int = { type_con_name = Name_prim.name_tp_int; type_con_kind = Kind.Inner.kind_star }
...
(** Type of floats *)
let type_float : tau = TCon({ type_con_name = Name_prim.name_tp_float; type_con_kind = Kind.Inner.kind_star})

let tcon_char = { type_con_name = Name_prim.name_tp_char; type_con_kind = Kind.Inner.kind_star}
...
let tcon_string = { type_con_name = Name_prim.name_tp_string; type_con_kind = Kind.Inner.kind_star};;
...
let effect_empty : tau =
  TCon({ type_con_name = Name_prim.name_effect_empty; type_con_kind = Kind.Inner.kind_effect })
...
let tcon_effect_extend : type_con =
  { type_con_name = Name_prim.name_effect_extend; type_con_kind = (Kind.Inner.kind_fun Kind.Inner.kind_label (Kind.Inner.kind_fun Kind.Inner.kind_effect Kind.Inner.kind_effect)) }
...
and effect_extends (labels : tau list) (eff : tau) : tau =
  match labels with
  | [TSyn({type_syn_kind=kind;_},_,_) as lab] when is_effect_empty eff && kind = Kind.Inner.kind_effect -> lab
...
let tcon_total = { type_con_name = Name_prim.name_effect_empty; type_con_kind = Kind.Inner.kind_effect }
...
let tcon_bool : type_con = { type_con_name = Name_prim.name_tp_bool; type_con_kind = Kind.Inner.kind_star }
...
let tcon_unit : type_con = { type_con_name = Name_prim.name_tp_unit; type_con_kind = Kind.Inner.kind_star }
...
let tcon_list : type_con = {
  ...
  type_con_kind = (Kind.Inner.kind_fun Kind.Inner.kind_star Kind.Inner.kind_star)
}
...
let type_void : tau = TCon { type_con_name = Name_prim.name_tp_void; type_con_kind = Kind.Inner.kind_star }

let type_tuple (n : int) : tau =
  TCon { type_con_name = (Name_prim.name_tuple n); type_con_kind = (Kind.Inner.kind_arrow_n n)}
...
let sexp_of_flavour = Kind.Inner.sexp_of_flavour
  (* function
   * | Bound  -> Sexp.Atom "Bound"
   * | Skolem -> Sexp.Atom "Skolem"
   * | Meta   -> Sexp.Atom "Meta"; *)

let flavour_of_sexp = Kind.Inner.flavour_of_sexp
