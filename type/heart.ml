...
and def_group = Recursive of def list
               | NonRecursive of def
...
let add_type_apps (ts: Type.type_var list) (e:expr) : expr =
  match (ts,e) with 
  | ([],_) -> e
  | (ts, TypeApp(e,args)) -> TypeApp(e, args @ List.map (fun t -> Type.TVar t) ts)
  | (ts, e) -> TypeApp(e, List.map (fun t -> Type.TVar t) ts)
