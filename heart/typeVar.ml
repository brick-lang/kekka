...
module C = Comparator.Make(struct
    type nonrec t = Type.TypeVar.t
    let compare = Type.TypeVar.compare
    let sexp_of_t = Type.TypeVar.sexp_of_t
  end)

module Set = Core.Set.Make_using_comparator(struct
    include C
    type nonrec t = Type.TypeVar.t
    let t_of_sexp = Type.TypeVar.t_of_sexp
    let sexp_of_t = Type.TypeVar.sexp_of_t
  end)
...
let rec show_tp =
  let open Type in function
    ...
    | Type.TCon tcon -> Name.show tcon.name ^ " : " ^ Kind.show tcon.kind
    | TApp(tp,args)  -> show_tp tp ^ "<" ^ String.concat ~sep:"," (List.map ~f:show_tp args) ^ ">"
    | TSyn(syn,args,body) -> "(syn:" ^ Name.show syn.type_syn_name ^ " : " ^ Kind.show syn.type_syn_kind
...
module type HasOrderedTypeVar = sig
  ...
  val odftv : t -> Type.TypeVar.t list
end
...
let sub_new (sub : (t * tau) list) : sub =
  Failure.assertwith (Printf.sprintf "TypeVar.sub_new.KindMisMatch: %i {%s}" (List.length sub)
    ...
    (List.for_all ~f:(fun (x,t) -> Kind.equal (TypeKind.get_kind_type_var x) (TypeKind.get_kind_typ t)) sub)
...
let sub_list : sub -> (Type.TypeVar.t * tau) list = Map.to_alist
...
let sub_find tvar sub : tau = match sub_lookup tvar sub with
  ...
  | Some tau ->
      Failure.assertwith ("Type.TypeVar.sub_find: incompatible kind: "
                          ^ Type.TypeVar.show tvar ^ ":"
                          ^ Kind.show (TypeKind.get_kind_type_var tvar) ^ ","
                          ^ "?" ^ ":" ^ Kind.show (TypeKind.get_kind_typ tau))
        (Kind.equal (TypeKind.get_kind_type_var tvar) (TypeKind.get_kind_typ tau)) @@
      tau
...
module rec HasTypeVar_pred : HasTypeVarEx with type t = Type.pred = struct
  open Type
...
and HasTypeVar_typ : HasTypeVarEx with type t = Type.typ = struct
  open Type
  type t = typ
...
and HasTypeVar_list_typ : HasTypeVarEx with type t = Type.typ list = HasTypeVar_list(HasTypeVar_typ)
and HasTypeVar_list_pred : HasTypeVarEx with type t = Type.pred list = HasTypeVar_list(HasTypeVar_pred)

module rec HasOrderedTypeVar_typ : HasOrderedTypeVar with type t = Type.typ = struct
  open Type
  type t = typ
  let odftv tp =
    let rec odftv' = function
      | TForall (vars, preds, tp) ->
          ...
          List.filter ~f:(fun tv -> not (List.mem vars tv ~equal:equal)) (odftv' tp @ HOTV_list_pred.odftv preds)
      | TFun (args, effect, result) ->
...
and HasOrderedTypeVar_pred : HasOrderedTypeVar with type t = Type.pred = struct
  type t = Type.pred
...
let sub_single tvar (tau:tau) : sub =
  ...
  |> Failure.assertwith "Type.TypeVar.sub_single.KindMismatch" (Kind.equal (TypeKind.get_kind_type_var tvar) (TypeKind.get_kind_typ tau))
...
let sub_extend (tvar:Type.TypeVar.t) (tau:tau) (sub:sub) =
  (sub_single tvar tau) @@@ sub

let fresh_type_var kind (flavour : Type.Flavour.t) =
  let open Kind in
  let id = Unique.unique_id (match flavour with Flavour.Meta -> "_v" | Flavour.Skolem -> "$v" | Flavour.Bound -> "v") in
  Type.TypeVar.{id; kind; flavour}
...
module rec HasTypeVar_def : HasTypeVar with type t = Expr.def = struct
  open Expr
  type t = def
...
and HasTypeVar_list_def : HasTypeVarEx with type t = Expr.def list
...
and HasTypeVar_list_defgroup : HasTypeVarEx with type t = Expr.def_group list
  = HasTypeVar_list(HasTypeVar_defgroup)

and HasTypeVar_pattern : HasTypeVar with type t = Expr.pattern = struct
  open Expr
  type t = pattern
...
and HasTypeVar_list_pattern : HasTypeVarEx with type t = Expr.pattern list
  = HasTypeVar_list(HasTypeVar_pattern)

and HasTypeVar_guard : HasTypeVar with type t = Expr.guard = struct
  open Expr
  type t = guard
...
and HasTypeVar_list_guard : HasTypeVarEx with type t = Expr.guard list
  = HasTypeVar_list(HasTypeVar_guard)

and HasTypeVar_branch : HasTypeVar with type t = Expr.branch = struct
  open Expr
  type t = branch
...
and HasTypeVar_list_branch : HasTypeVarEx with type t = Expr.branch list
  = HasTypeVar_list(HasTypeVar_branch)

and HasTypeVar_expr : HasTypeVarEx with type t = Expr.expr = struct
  open Expr
  type t = expr
  let rec substitute sub = function
...
and HasTypeVar_list_expr : HasTypeVarEx with type t = Expr.expr list = HasTypeVar_list(HasTypeVar_expr)
