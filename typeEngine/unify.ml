...
let rec match_kind kind1 kind2 : bool = match kind1, kind2 with
  | Kind.Constant(c1), Kind.Constant(c2) -> Name.equal c1 c2
...
let rec overlaps (_free:TypeVar.Set.t) (tp1:Type.typ) (tp2:Type.typ) : unit UnifyM.t =
  ...
  | (Some(targs1,_,_),Some(targs2,_,_)) ->
      ...
      let hi = Stdlib.max (List.length fixed1) (List.length fixed2) in
      let fo1 = (flip List.take) hi (fixed1 @ (List.map ~f:Type.unoptional optional1)) in
      let fo2 = (flip List.take) hi (fixed2 @ (List.map ~f:Type.unoptional optional2)) in
...
(** Unify two types. *)
and unify (t1:Type.typ) (t2:Type.typ) : unit UnifyM.t = let open UnifyM in match (t1,t2) with
  (* Effects *)
  | TApp(TCon tc1, _), TApp(TCon tc2, _)
    when (Type.TypeCon.equal tc2 Type.tcon_effect_extend) &&
         (Type.TypeCon.equal tc1 Type.tcon_effect_extend) ->
      unify_effect t1 t2
        
  | TApp(TCon tc1, _), TVar tv2
    when (Type.TypeCon.equal tc1 Type.tcon_effect_extend) &&
         Type.is_meta tv2 ->
      unify_effect_var tv2 t1
        
  | TVar tv1, TApp(TCon tc2, _)
    when (Type.TypeCon.equal tc2 Type.tcon_effect_extend) &&
         Type.is_meta tv1 ->
      unify_effect_var tv1 t2
  
  (* Type variables *)
  | (TVar v1, TVar v2) when Type.TypeVar.equal v1 v2 -> return ()
  | (TVar tv, tp) when Type.is_meta tv -> unify_tvar tv tp
  | (tp, TVar tv) when Type.is_meta tv -> unify_tvar tv tp

  (* Constants *)
  | TCon tc1, TCon tc2 when Type.TypeCon.equal tc1 tc2 -> return ()
...
and unifies (tl1:Type.typ list) (tl2:Type.typ list) : unit UnifyM.t = let open UnifyM in
  match (tl1,tl2) with
  | [], [] -> UnifyM.return ()
  | t::ts, u::us ->
...
and unify_effect_var tv1 tp2  = let open UnifyM in
  ...
  | Type.TVar tv2 when Type.TypeVar.equal tv1 tv2 ->  (* e ~ <div,exn|e> ~> e := <div,exn|e'> *)
      error Infinite
  | _ ->
...
and unify_tvar (tv:Type.TypeVar.t) (tp:Type.typ) : unit UnifyM.t =
  ...
  if TypeVar.tvs_member (TypeVar.tvs_filter ~f:Type.is_meta (TypeVar.HasTypeVar_typ.ftv etp)) tv then
    ...
    | Type.TVar tv2 when Type.TypeVar.equal tv tv2 -> UnifyM.return () (* i.e. a ~ id<a> *)
    | _ -> UnifyM.error Infinite
  else
    match etp with
    | Type.TVar{flavour=Type.Flavour.Bound; _} -> UnifyM.error NoMatch (* can't unify with bound variables *)
    | Type.TVar({id=id2; flavour=Type.Flavour.Meta; _} as tv2) when tv.Type.TypeVar.id <= id2 ->
...
and unify_labels (ls1:Type.tau list) (ls2:Type.tau list) : (Type.tau list * Type.tau list) UnifyM.t = let open UnifyM in
  ...
  | ([], _::_) -> return (ls2,[])
  | (l1::ll1, l2::ll2) ->
      let compared = Name.compare (Type.label_name l1) (Type.label_name l2) in
      if compared < 0 then
        ...
        unify l1 l2 >>
        let%bind _ll1' = (get_subst >>= fun (sub) -> return TypeVar.HasTypeVar_list_typ.(sub |-> ll1)) in
        let%bind _ll2' = (get_subst >>= fun (sub) -> return TypeVar.HasTypeVar_list_typ.(sub |-> ll2)) in
...
and unify_preds ps1 ps2 = let open UnifyM in
  ...
  | [],[] -> return ()
  | p1::ps1, p2::ps2 ->
      let%bind _sp1 = subst_pred p1 in 
      let%bind _sp2 = subst_pred p2 in
      unify_pred p1 p2 >>
      unify_preds ps1 ps2
...
let rec entails (skolems:TypeVar.Set.t) (known:Evidence.t list) (preds:Evidence.t list) =
  match preds with
  | [] -> UnifyM.return ([],id)
  (* TODO: possible failure point here 
   * TODO: should construct evidence from known to preds (simple one-to-one name mapping)*)
  | evs when List.equal Type.Eq_pred.equal
  ...
  | ev::evs -> match ev.Evidence.pred with
    | PredIFace(name,[_;_;_]) when (Name.equal name Name.pred_heap_div) -> (* can always be solved *)
...
let match_arguments (match_some:bool) (* (range:range) *) (free:TypeVar.Set.t) (tp:Type.typ) (fixed:Type.typ list) (named:(Name.t * Type.typ) list) : unit UnifyM.t =
  ...
  | Some(pars,_,_) ->
      if ((List.length fixed) + (List.length named)) > (List.length pars) then
        ...
        mapM_ (fun (tpar,targ) -> subsume free (Type.unoptional tpar) targ) (List.zip_exn (List.map ~f:snd fpars) fixed) >>
        (* subsume named parameters *)
        forM_ named (fun (name,targ) -> match List.Assoc.find npars name ~equal:Name.equal with
            | None -> error NoMatch
